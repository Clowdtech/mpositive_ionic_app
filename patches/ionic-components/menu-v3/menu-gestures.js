...
var MenuContentGesture = (function (_super) {
    ...
    MenuContentGesture.prototype.onSlide = function (slide) {
        var /** @type {?} */ z = z = (this.menu.side === 'right' || this.menu.side === 'bottom' ? slide.min : slide.max);
    ...
    MenuContentGesture.prototype.onSlideEnd = function (slide) {
      ...
      var shouldCompleteRight = shouldCompleteBottom = (velocity >= 0) && (velocity > 0.2 || slide.delta > z);
        this.menu._swipeEnd(shouldCompleteLeft, shouldCompleteRight, currentStepValue, velocity);
      var shouldCompleteTop;
      var shouldCompleteLeft = shouldCompleteTop = (velocity <= 0) && (velocity < -0.2 || slide.delta < -z);
      this.menu._swipeEnd(shouldCompleteLeft, shouldCompleteRight,
          shouldCompleteTop, shouldCompleteBottom,
    ...
    MenuContentGesture.prototype.getElementStartPos = function (slide) {
        var /** @type {?} */ menu = this.menu;
        if (menu.side === 'right' || menu.side === 'bottom') {
